...
#   1st: ckeck, if the following energy is in kinetic (KE) or binding (BE) energy. 
...
#
#   select x values
#   select y values from datas X´s column
#   use this data to plot line plot
#   calculate y values for 2nd line
#   add 2nd data (line)
#   plot plot
#   check if plot is specifiet. If plot = true, terminate programm, else calculate matrix
# 
##########################
# calculate matrix
# 
#   create result array /open already exisitng one
#   for every column do:
#       give boundaries for lin bg
#       calculate lin BG
#       give in right boundarys for integration
#       subtract from curve
#       integrate
...
def choice_fkt(choice_input):                       # function to get the possibilities for the follwoing question do you want to:
    if choice_input == "yes":                       # do the 1st part of the script (plotting and selectring the regions)
        plot_matrix_choice = True
        return plot_matrix_choice
    if choice_input == "no":                        # create the Martix
        plot_matrix_choice = True
        return plot_matrix_choice
...
def lin_BG_int_fkt():                              # fkt to calculate the lin BG
    ...
    while j <= spectra_nr:
        i=0
        correct_range = energy_range_limits_int_fkt(fit_lower_E_input, j)
        fit_lower_E=correct_range[1]                                        # to get the E value
        fit_lower_E_spectra = correct_range[2]                              # to get the actual spectra value
        int_lower_E_position = correct_range[3]   
        
        correct_range = energy_range_limits_int_fkt(fit_upper_E_input, j)         
...
def integration_fkt(j):
    #getting the postions of the lower and higher integrations
    ...
    l=m-1        # this is only possible, if the luwer boundary is not the first value
    o=n
    while m <= n:
        df4.iat[m,j] = df4.iat[m,j]+df4.iat[l,j]
        l +=1
        m +=1
        if m==n:
            print("The integral for spectra" ,j, "=", df4.iat[int_lower_E_position,j])
...
spectra_nr = input("How many spectra (columns, energy not inlcuded) are in the file?\n")
spectra_nr=int(spectra_nr)

#################################################################
# plot the plot

if plot_or_calculation == "yes":
    ...
    while int_range == False:                              
        ...
        if int_range_input == 'no': 
            # to check if, after htis run, the boundaries are good enough     
...
if plot_or_calculation == 'no':
    ...
    if plot_or_calculation_continue !='yes':  
        ...
        k=0
        while correct_int_range == False:
            if k ==0: print("Are", fit_lower_E,"eV &",fit_upper_E ,"eV the same boundaries for integration? \n")
            if k > 0: print("Are", int_lower_E,"eV &",int_upper_E ,"eV the right boundaries for integration? \n")
            correct_int_range_input = input("If yes: type 'yes'. If not 'no'\n")
            if correct_int_range_input != 'yes' and correct_int_range_input != 'no':
                print("\nError, please type in 'yes' or 'no'\n")
                correct_int_range = False
    ...
    j=1                                     # sum up the values (integrate) for all spectra j
    while j <= spectra_nr:
        integration_fkt(j)
        j +=1
    ...
    j=1
    while j <= spectra_nr:  
        if j ==1:sns.lineplot(x="E", y=j, data=df4, color=(0,0,255))  
        if j ==2:sns.lineplot(x="E", y=j, data=df4, color=(30,60,255))    
        if j ==3:sns.lineplot(x="E", y=j, data=df4, color=(94,144,255))  
        if j ==4:sns.lineplot(x="E", y=j, data=df4, color=(157,60,255))  
        if j ==5:sns.lineplot(x="E", y=j, data=df4, color=(214,49,255)) 
        if j ==6:sns.lineplot(x="E", y=j, data=df4, color=(240,00,130))  
        if j ==7:sns.lineplot(x="E", y=j, data=df4, color=(255,0,0))  
        if j ==8:sns.lineplot(x="E", y=j, data=df4, color=(240,130,40)) 
        if j ==9:sns.lineplot(x="E", y=j, data=df4, color=(230,175,45))  
        if j ==10:sns.lineplot(x="E", y=j, data=df4, color=(230,220,50))  
        if j ==11:sns.lineplot(x="E", y=j, data=df4, color=(0,220,0)) 
        if j ==12:sns.lineplot(x="E", y=j, data=df4, color=(60,255,60)) 
        if j ==12:sns.lineplot(x="E", y=j, data=df4, color=(0,255,0)) 
        j +=1
    plt.axvline(x=fit_lower_E, label='lower E range for fit',color='blue')
    plt.axvline(x=fit_upper_E, label='upper E range for fit',color='blue')
    plt.axvline(x=int_lower_E, label='lower E range for int',color='red')
